...
function executeRank(){
    ...
    let carbon1 = 0;
    let protein1 =0;
    for(var j = 0; j< userlist[i].meals.length; j++){//iterate food and exerice and put them in above things


        crawler.getCaloriesPerMeal(userlist[i].meals[j], (calories,carbs,protein,fat) => {
...
var joe = new Profile("joe",180,79,19,1,"male");
    joe.save();
    joe.addMeal("banana");
    joe.addActivity("run");

var joe1 = new Profile("joe1",189,79,19,1.3,"male");
    joe.save();
    joe.addMeal("apple");
    joe.addActivity("swim");

var joe2 = new Profile("joe2",180,79,19,2.1,"male");
    joe.save();
    joe.addMeal("orange");
